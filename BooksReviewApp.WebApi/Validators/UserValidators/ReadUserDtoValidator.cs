...
namespace BooksReviewApp.WebApi.Validators.UserValidators
{
    public class ReadUserDtoValidator : AbstractValidator<ReadUserDto>
    {
        public ReadUserDtoValidator()
        {
            ...
            RuleFor(dto => dto.Email).NotEmpty().MaximumLength(UserValidation.MaxEmailLength).EmailAddress().MinimumLength(20);
            RuleFor(dto => dto.Password).NotEmpty().MaximumLength(UserValidation.MaxPasswordLength).MinimumLength(10);
